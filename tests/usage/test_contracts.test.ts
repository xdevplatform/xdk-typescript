...
describe('UsageClient Contracts', () => {
  let client: Client;
  ...
  it('should have correct request structure for get', async () => {
    ...
    try {
      ...
      // Call the method
      const method = (usageClient as any)['get'];
      const result = await method.apply(usageClient, [...requiredArgs, options]);
      ...
      const expectedPath = '/2/usage/tweets';
  ...
  it('should handle required parameters correctly for get', async () => {
    ...
    try {
      ...
      await expect(method.apply(usageClient, [...requiredArgs, options])).resolves.toBeDefined();
