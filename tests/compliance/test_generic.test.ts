...
describe('ComplianceClient Generic', () => {
  let client: Client;
  ...
  it('should have ComplianceClient class that is importable', () => {
    expect(ComplianceClient).toBeDefined();
    expect(typeof ComplianceClient).toBe('function');
  });

  it('should initialize ComplianceClient properly', () => {
    expect(complianceClient).toBeDefined();
    expect(complianceClient).toBeInstanceOf(ComplianceClient);
  });

  it('should have expected base functionality', () => {
    // Should be able to access the client through main Client
    expect(client).toHaveProperty('compliance');
    
    // Client should have standard object features
    expect(complianceClient).toHaveProperty('constructor');
    
    // Should have at least one public method
    const publicMethods = Object.getOwnPropertyNames(ComplianceClient.prototype)
  ...
  it('should handle error responses correctly', async () => {
    ...
    try {
      ...
      if (publicMethods.length > 0) {
        ...
        const method = (complianceClient as any)[methodName];
        
        // Try calling the method and expect an ApiError
        await expect(method.call(complianceClient)).rejects.toThrow(ApiError);
      }
    } finally {
  ...
  it('should have valid method signatures', () => {
    ...
    for (const methodName of publicMethods) {
      const method = (ComplianceClient.prototype as any)[methodName];
