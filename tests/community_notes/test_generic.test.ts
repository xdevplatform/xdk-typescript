 ...
 * Any manual changes will be overwritten on the next generation.
 * 
 * Auto-generated generic tests for CommunityNotes client.
 *
...
describe('CommunityNotesClient Generic', () => {
  let client: Client;
  ...
  it('should have CommunityNotesClient class that is importable', () => {
    expect(CommunityNotesClient).toBeDefined();
    expect(typeof CommunityNotesClient).toBe('function');
  });

  it('should initialize CommunityNotesClient properly', () => {
    expect(communityNotesClient).toBeDefined();
    expect(communityNotesClient).toBeInstanceOf(CommunityNotesClient);
  });

  it('should have expected base functionality', () => {
    // Should be able to access the client through main Client
    expect(client).toHaveProperty('communityNotes');
    
    // Client should have standard object features
    expect(communityNotesClient).toHaveProperty('constructor');
    
    // Should have at least one public method
    const publicMethods = Object.getOwnPropertyNames(CommunityNotesClient.prototype)
  ...
  it('should handle error responses correctly', async () => {
    ...
    try {
      ...
      if (publicMethods.length > 0) {
        ...
        const method = (communityNotesClient as any)[methodName];
        
        // Try calling the method and expect an ApiError
        await expect(method.call(communityNotesClient)).rejects.toThrow(ApiError);
      }
    } finally {
  ...
  it('should have valid method signatures', () => {
    ...
    for (const methodName of publicMethods) {
      const method = (CommunityNotesClient.prototype as any)[methodName];
