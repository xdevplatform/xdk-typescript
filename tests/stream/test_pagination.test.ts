...
describe('StreamClient Pagination', () => {
  let client: Client;
  ...
  it('should create paginator for getRules', () => {
  ...
  it('should paginate through pages for getRules', async () => {
    ...
    try {
      ...
      const firstPage = await method.apply(streamClient, [...requiredArgs, options]);
      ...
      if (firstPage.meta && firstPage.meta['next_token']) {
        ...
        const secondPage = await method.apply(streamClient, [...requiredArgs, options]);
  ...
  it('should handle pagination parameters correctly for getRules', async () => {
    // Mock validateAuthentication to bypass auth checks (like Python mocks session)
    const originalValidateAuth = client.validateAuthentication;
    client.validateAuthentication = jest.fn();
    ...
    try {
      ...
      await method.apply(streamClient, [...requiredArgs, options]);
