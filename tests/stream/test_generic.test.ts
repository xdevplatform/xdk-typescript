...
describe('StreamClient Generic', () => {
  ...
  it('should have StreamClient class that is importable', () => {
    expect(StreamClient).toBeDefined();
    expect(typeof StreamClient).toBe('function');
  });

  it('should initialize StreamClient properly', () => {
    expect(streamClient).toBeDefined();
  ...
  it('should have expected base functionality', () => {
    ...
    expect(streamClient).toHaveProperty('constructor');
    
    // Should have at least one public method
    const publicMethods = Object.getOwnPropertyNames(StreamClient.prototype)
  ...
  it('should handle error responses correctly', async () => {
    ...
    try {
      ...
      if (publicMethods.length > 0) {
        ...
        const method = (streamClient as any)[methodName];
        
        // Try calling the method and expect an ApiError
        await expect(method.call(streamClient)).rejects.toThrow(ApiError);
      }
    } finally {
  ...
  it('should have valid method signatures', () => {
    ...
    for (const methodName of publicMethods) {
      const method = (StreamClient.prototype as any)[methodName];
