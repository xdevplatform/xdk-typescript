...
describe('CommunitiesClient Generic', () => {
  let client: Client;
  ...
  it('should have CommunitiesClient class that is importable', () => {
    expect(CommunitiesClient).toBeDefined();
    expect(typeof CommunitiesClient).toBe('function');
  });

  it('should initialize CommunitiesClient properly', () => {
    expect(communitiesClient).toBeDefined();
    expect(communitiesClient).toBeInstanceOf(CommunitiesClient);
  });

  it('should have expected base functionality', () => {
    // Should be able to access the client through main Client
    expect(client).toHaveProperty('communities');
    
    // Client should have standard object features
    expect(communitiesClient).toHaveProperty('constructor');
    
    // Should have at least one public method
    const publicMethods = Object.getOwnPropertyNames(CommunitiesClient.prototype)
  ...
  it('should handle error responses correctly', async () => {
    ...
    try {
      ...
      if (publicMethods.length > 0) {
        ...
        const method = (communitiesClient as any)[methodName];
        
        // Try calling the method and expect an ApiError
        await expect(method.call(communitiesClient)).rejects.toThrow(ApiError);
      }
    } finally {
  ...
  it('should have valid method signatures', () => {
    ...
    for (const methodName of publicMethods) {
      const method = (CommunitiesClient.prototype as any)[methodName];
